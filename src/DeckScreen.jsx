...
function DeckSettingsDropdown({ onDelete, onExport, lightMode, onToggleLight }) {
      ...
      {open && (
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", background: "var(--lc-dropBg)", border: `1px solid var(--lc-modalBorder)`, borderRadius: 12, overflow: "hidden", minWidth: 200, zIndex: 50, boxShadow: `0 8px 28px var(--lc-shadow)` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderTop: `1px solid ${C.border}` }}>
            ...
            <span style={{ flex: 1, fontSize: 13, color: C.textDim, fontFamily: "'Lora',serif" }}>Světlý režim</span>
            <div onClick={onToggleLight}
              style={{ width: 46, height: 26, borderRadius: 13, flexShrink: 0, background: lightMode ? "var(--lc-ok)" : "var(--lc-mutedDark)", position: "relative", cursor: "pointer", transition: "background .25s" }}>
              <div style={{ position: "absolute", top: 3, left: lightMode ? 23 : 3, width: 20, height: 20, borderRadius: "50%", background: "white", transition: "left .25s", boxShadow: "0 1px 4px rgba(0,0,0,.4)" }} />
            </div>
          </div>
          <div style={{ borderTop: `1px solid ${C.border}` }} />
...
export default function DeckScreen({
  ...
  const statItems = [
    { lbl: "Úspěšnost", val: sr !== null ? `${sr}%` : "—", c: "#7090c8", bg: "var(--lc-statBg1)" },
    { lbl: "Naučeno", val: `${mastered}/${deck.words.length}`, c: C.ok, bg: C.okBg },
    { lbl: "Procvičeno", val: `${practiced}/${deck.words.length}`, c: "#a080c8", bg: "var(--lc-statBg2)" },
    { lbl: "K opak.", val: due, c: due > 0 ? "var(--lc-dueText)" : C.ok, bg: due > 0 ? "var(--lc-dueBg)" : C.okBg },
    { lbl: "Odpovědí", val: ds.totalAnswers || 0, c: C.gold, bg: "var(--lc-statBg5)" },
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: "var(--lc-headerBg)", borderBottom: `1px solid var(--lc-headerBorder)`, padding: "0.8rem 1rem" }}>
        <div style={{ maxWidth: 1020, margin: "0 auto", display: "flex", flexDirection: "column", gap: 7 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ flex: 1 }} />
            <button className="btn" onClick={() => setShowStats(true)} style={{ border: `1px solid var(--lc-statBg1)`, color: "#7090c8", borderRadius: 8, padding: "5px 11px", fontSize: 12, cursor: "pointer", flexShrink: 0 }}>📊 Stat.</button>
            <button className="btn" onClick={() => setShowAdd(true)} style={{ border: `1px solid var(--lc-btnAddBorder)`, color: C.muted, borderRadius: 8, padding: "5px 11px", fontSize: 12, cursor: "pointer", flexShrink: 0 }}>+ Slovo</button>
            <DeckSettingsDropdown onDelete={() => setShowDelDeck(true)} onExport={onExport} lightMode={lightMode} onToggleLight={onToggleLight} />
          </div>
